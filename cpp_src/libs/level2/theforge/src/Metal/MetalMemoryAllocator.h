...
struct AllocatorMutexLock {
  public:
      AllocatorMutexLock(RESOURCE_MUTEX & mutex, bool
  useMutex):
  m_pMutex(useMutex
  ? &
  mutex : RESOURCE_NULL
...
typename T
>
typename AllocatorPoolAllocator<T>::ItemBlock
&
AllocatorPoolAllocator<T>::CreateNewBlock() {
...
struct ResourceAllocation {
  ...
  id <MTLHeap> GetMemory()
  const;
  id <MTLBuffer> GetResource()
  const;
  uint32_t GetMemoryTypeIndex()
  const;
  RESOURCE_BLOCK_VECTOR_TYPE GetBlockVectorType()
  const;
  void *GetMappedData()
  const;
...
struct AllocatorBlockVector {
  ...
  AllocatorVector<AllocatorBlock*>
  m_Blocks;

  AllocatorBlockVector(ResourceAllocator
  * hAllocator);
  ~
...
struct ResourceAllocator {
  ...
  uint64_t GetPreferredBlockSize(ResourceMemoryUsage memUsage, uint32_t memTypeIndex)
  const;
  ...
  const { return RESOURCE_MEMORY_TYPE_NUM_TYPES; }
...
inoutInfo.
SuballocationSizeAvg =
(inoutInfo.SuballocationCount > 0) ? AllocatorRoundDiv < uint64_t > (inoutInfo.UsedBytes, inoutInfo.SuballocationCount)
                                   : 0;
inoutInfo.
UnusedRangeSizeAvg =
...
id <MTLHeap> ResourceAllocation::GetMemory()
const {
return (m_Type == ALLOCATION_TYPE_BLOCK) ? m_BlockAllocation.m_Block->m_hMemory :
nil;
}

id <MTLBuffer> ResourceAllocation::GetResource()
const
{
...
RESOURCE_BLOCK_VECTOR_TYPE ResourceAllocation::GetBlockVectorType()
const
{
...
void *ResourceAllocation::GetMappedData()
const
{
switch (m_Type)
{
...
break;
case ALLOCATION_TYPE_OWN: return m_OwnAllocation.
m_pMappedData;
...
AllocatorBlock::AllocatorBlock(ResourceAllocator
* hAllocator):
...
AllocatorBlockVector::AllocatorBlockVector(ResourceAllocator
* hAllocator):
...
{
m_pBlockVectors[i][j] =
conf_placement_new<AllocatorBlockVector>(AllocatorAllocate<AllocatorBlockVector>(), this
);
m_pOwnAllocations[i][j] =
conf_placement_new<AllocationVectorType> (AllocatorAllocate<AllocationVectorType>());
}
}
}
...
for (
size_t allocIndex = 0;
allocIndex < blockVector->m_Blocks.
size();
++allocIndex)
{
...
if (pBlock->
CreateAllocationRequest(GetBufferImageGranularity(), mtlMemReq
.size, mtlMemReq.align, suballocType, &allocRequest))
{
...
{
m_HasEmptyBlock[memTypeIndex] =
false;
}
// Allocate from this pBlock.
pBlock->
Alloc(allocRequest, suballocType, mtlMemReq
.size);
*
...
InitBlockAllocation(
    pBlock, allocRequest
.offset, mtlMemReq.align, mtlMemReq.size, suballocType, resourceAllocMemReq.pUserData);
RESOURCE_HEAVY_ASSERT(pBlock->Validate());
...
MTLHeapDescriptor *allocInfo =
[[
MTLHeapDescriptor alloc
] init];
allocInfo.
size = preferredBlockSize;
allocInfo.
cpuCacheMode = pHeapProps->mCPUCacheMode;
allocInfo.
storageMode = pHeapProps->mStorageMode;

// Start with full preferredBlockSize.
id <MTLHeap> mem = nil;
mem =
[
m_Device newHeapWithDescriptor
:allocInfo];
mem.
label =
[
...
pBlock->
Init(memTypeIndex, (RESOURCE_BLOCK_VECTOR_TYPE)
blockVectorType, mem, allocInfo.size, false, NULL);
...
AllocatorAllocationRequest allocRequest = {};
allocRequest.
freeSuballocationItem = pBlock->m_Suballocations.begin();
allocRequest.
offset = 0;
pBlock->
Alloc(allocRequest, suballocType, mtlMemReq
...
uint64_t ResourceAllocator::GetPreferredBlockSize(ResourceMemoryUsage memUsage, uint32_t memTypeIndex)
const
{
return gHeapProperties[memTypeIndex].
mBlockSize;
}
...
void ResourceAllocator::PrintDetailedMap(AllocatorStringBuilder
& sb)
{
...
void resourceAllocBuildStatsString(ResourceAllocator *allocator, char **ppStatsString, uint32_t detailedMap) {
  ...
  char *const pChars = conf_placement_new<char>(AllocatorAllocateArray <
  char>((len + 1)));
  if (len > 0) {
