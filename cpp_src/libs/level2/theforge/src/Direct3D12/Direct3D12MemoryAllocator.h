...
*/
static inline bool AllocatorBlocksOnSamePage(UINT64 resourceAOffset,
...
AllocatorRawList<T>::AllocatorRawList()
    : m_ItemAllocator(128), m_pFront(RESOURCE_NULL), m_pBack(RESOURCE_NULL), m_Count(0) {
}
...
struct ResourceAllocator {
  ...
  UINT64 GetBufferImageGranularity() const {
    return RESOURCE_MAX(static_cast<UINT64>(RESOURCE_DEBUG_MIN_BUFFER_IMAGE_GRANULARITY),
                        1);
  }
