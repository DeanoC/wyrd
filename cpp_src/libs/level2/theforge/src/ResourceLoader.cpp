...
#ifndef METAL
static void calculate_upload_texture_params(ImageFormat::Enum fmt,
                                            uint32_t w,
                                            uint32_t h,
                                            UploadTextureParams& params) {
  switch (ImageFormat::GetBlockSize(fmt)) {
    case ImageFormat::BLOCK_SIZE_4x4: params.w = (w + 3) & 0xFFFFFFFC;
      params.stridePitch = (params.w >> 2) * ImageFormat::GetBytesPerBlock(fmt);
      params.h = (h + 3) & 0xFFFFFFFC;
      params.numStrides = params.h >> 2;
      break;
    case ImageFormat::BLOCK_SIZE_4x8: params.w = (w + 7) & 0xFFFFFFF8;
...
void addResource(TextureLoadDesc *pTextureDesc, SyncToken *token) {
  ...
  mbstowcs(debugName, filename.c_str(), Math_MinU64((size_t) MAX_PATH, filename.size()));
