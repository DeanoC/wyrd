...
void cmdBeginGpuTimestampQuery(Cmd *pCmd,
                               struct GpuProfiler *pGpuProfiler,
                               const char *pName,
                               bool addMarker,
                               const float3& color) {
  // hash name
...
void cmdEndGpuFrameProfile(Cmd *pCmd, GpuProfiler *pGpuProfiler) {
  ...
  for (uint32_t i = 0; i < (uint32_t) pGpuProfiler->mRoot.mChildren.size(); ++i) {
    pGpuProfiler->mCumulativeTimeInternal +=
        getAverageGpuTime(pGpuProfiler, &pGpuProfiler->mRoot.mChildren[i]->mGpuTimer);

    pGpuProfiler->mCumulativeCpuTimeInternal +=
        getAverageCpuTime(pGpuProfiler, &pGpuProfiler->mRoot.mChildren[i]->mGpuTimer);
  }
