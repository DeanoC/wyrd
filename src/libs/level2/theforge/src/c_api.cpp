...
EXTERN_C void TheForge_InitRenderer(const char *app_name, const RendererDesc *p_settings,Renderer **ppRenderer)
...
{ AddSemaphore(pRenderer, pp_semaphore); }
...
{ AddQueue(pRenderer, pQDesc, ppQueue); }

EXTERN_C void TheForge_RemoveQueue(Queue *pQueue)
{ RemoveQueue(pQueue); }

EXTERN_C void TheForge_AddSwapChain(Renderer *pRenderer,const SwapChainDesc *p_desc, SwapChain **pp_swap_chain)
...
{ RemoveSwapChain(pRenderer, p_swap_chain); }

EXTERN_C void TheForge_AddCmdPool(Renderer *pRenderer, Queue *p_queue, bool transient, CmdPool **pp_CmdPool)
...
{ AddCmd(p_CmdPool, secondary, pp_cmd); }
...
{ AddCmd_n(p_CmdPool, secondary, cmd_count, ppp_cmd); }

EXTERN_C void TheForge_RemoveCmd_n(CmdPool *p_CmdPool, uint32_t cmd_count, Cmd **pp_cmd)
{ RemoveCmd_n(p_CmdPool, cmd_count, pp_cmd); }

EXTERN_C void TheForge_AddRenderTarget(Renderer *pRenderer, const RenderTargetDesc *p_desc, RenderTarget **pp_render_target)
{ AddRenderTarget(pRenderer, p_desc, pp_render_target); }
...
{ AddSampler(pRenderer, pDesc, pp_sampler); }
...
{ AddShader(pRenderer, p_desc, p_shader_program); }

EXTERN_C void TheForge_AddShaderBinary(Renderer *pRenderer,const BinaryShaderDesc *p_desc,Shader **p_shader_program)
{ AddShaderBinary(pRenderer, p_desc, p_shader_program); }

EXTERN_C void TheForge_RemoveShader(Renderer *pRenderer, Shader *p_shader_program)
{ RemoveShader(pRenderer, p_shader_program); }

EXTERN_C void TheForge_AddRootSignature(Renderer *pRenderer,const RootSignatureDesc *pRootDesc,RootSignature **pp_root_signature)
{ AddRootSignature(pRenderer, pRootDesc, pp_root_signature); }

EXTERN_C void TheForge_RemoveRootSignature(Renderer *pRenderer, RootSignature *pRootSignature)
{ RemoveRootSignature(pRenderer, pRootSignature); }

EXTERN_C void TheForge_AddPipeline(Renderer *pRenderer,const GraphicsPipelineDesc *p_pipeline_settings,Pipeline **pp_pipeline)
{ AddPipeline(pRenderer, p_pipeline_settings, pp_pipeline); }

EXTERN_C void TheForge_AddComputePipeline(Renderer *pRenderer,const ComputePipelineDesc *p_pipeline_settings,Pipeline **pp_pipeline)
...
EXTERN_C void TheForge_AddBlendState(Renderer *pRenderer, const BlendStateDesc *pDesc,BlendState **ppBlendState)
...
EXTERN_C void TheForge_AddDepthState(Renderer *pRenderer, const DepthStateDesc *pDesc, DepthState **ppDepthState)
...
EXTERN_C void TheForge_AddRasterizerState(Renderer *pRenderer, const RasterizerStateDesc *pDesc, RasterizerState **ppRasterizerState)
{ AddRasterizerState(pRenderer, pDesc, ppRasterizerState); }

EXTERN_C void TheForge_RemoveRasterizerState(Renderer *pRenderer, RasterizerState *pRasterizerState)
{ RemoveRasterizerState(pRenderer, pRasterizerState); }

EXTERN_C void TheForge_BeginCmd(Cmd *p_cmd)
{ BeginCmd(p_cmd); }

EXTERN_C void TheForge_EndCmd(Cmd *p_cmd)
{ EndCmd(p_cmd); }
...
{ CmdBindRenderTargets(p_cmd, render_target_count, pp_render_targets, p_depth_stencil, loadActions, pColorArraySlices, pColorMipSlices,depthArraySlice, depthMipSlice); }
...
EXTERN_C void TheForge_CmdSetScissor(Cmd *p_cmd, uint32_t x, uint32_t y, uint32_t width, uint32_t height)
...
{ CmdBindDescriptors(pCmd, pRootSignature, numDescriptors, pDescParams); }
...
EXTERN_C void TheForge_CmdBindVertexBuffer(Cmd *p_cmd,
                         ...
                         Buffer **pp_buffers,
                         uint64_t *pOffsets)
{ CmdBindVertexBuffer(p_cmd, buffer_count, pp_buffers, pOffsets); }
...
{ CmdDrawIndexed(p_cmd, index_count, first_index, first_vertex); }
...
EXTERN_C void TheForge_CmdResourceBarrier(Cmd *p_cmd,
                        ...
                        bool batch)
{ CmdResourceBarrier(p_cmd, buffer_barrier_count, p_buffer_barriers, texture_barrier_count, p_texture_barriers, batch); }
...
EXTERN_C void TheForge_CmdSynchronizeResources(Cmd *p_cmd,
                             ...
                             Texture **p_textures,
                             bool batch)
{ CmdSynchronizeResources(p_cmd, buffer_count, p_buffers, texture_count, p_textures, batch); }
...
{ AcquireNextImage(pRenderer, p_swap_chain, p_signal_semaphore, p_fence, p_image_index); }
...
{ QueueSubmit(p_queue, cmd_count, pp_cmds, pFence, wait_semaphore_count, pp_wait_semaphores, signal_semaphore_count, pp_signal_semaphores); }
...
EXTERN_C void TheForge_QueuePresent(Queue *p_queue,
                  ...
                  uint32_t wait_semaphore_count,
                  Semaphore **pp_wait_semaphores)
{ QueuePresent(p_queue, p_swap_chain, swap_chain_image_index, wait_semaphore_count, pp_wait_semaphores); }
...
{ GetFenceStatus(pRenderer, p_fence, p_fence_status); }
...
{ AddIndirectCommandSignature(pRenderer, p_desc, ppCommandSignature); }
...
{ CmdExecuteIndirect(pCmd, pCommandSignature,maxCommandCount, pIndirectBuffer, bufferOffset, pCounterBuffer, counterBufferOffset); }
...
EXTERN_C void TheForge_CmdResolveQuery(Cmd *pCmd,
                     ...
                     uint32_t startQuery,
                     uint32_t queryCount)
{ CmdResolveQuery(pCmd, pQueryHeap, pReadbackBuffer, startQuery, queryCount); }
...
EXTERN_C void TheForge_CmdBeginDebugMarker(Cmd *pCmd, float r, float g, float b, const char *pName)
...
EXTERN_C void TheForge_AllocBuffer(Renderer *pRenderer, const BufferDesc *pDesc, Buffer **pp_buffer)
{ AllocBuffer(pRenderer, pDesc, pp_buffer); }

EXTERN_C void TheForge_FreeBuffer(Renderer *pRenderer, Buffer *pBuffer)
{ FreeBuffer(pRenderer, pBuffer); }

EXTERN_C void TheForge_AllocTexture(Renderer *pRenderer, const TextureDesc *pDesc, Texture **ppTexture)
{ AllocTexture(pRenderer, pDesc, ppTexture); }

EXTERN_C void TheForge_FreeTexture(Renderer *pRenderer, Texture *pTexture)
{ FreeTexture(pRenderer, pTexture); }
