#pragma once
#ifndef WYRD_THEFORGE_METAL_MEMORYALLOCATOR_HPP
#define WYRD_THEFORGE_METAL_MEMORYALLOCATOR_HPP
...
template<typename T>
static inline void resource_swap(T& a, T& b) {
  T c = a;
  a = b;
  b = c;
}

#define RESOURCE_MIN(v1, v2) (v1 < v2 ? v1 : v2)
#define RESOURCE_MAX(v1, v2) (v1 > v2 ? v1 : v2)
...
#define RESOURCE_MUTEX Os_Mutex_t
#endif
...
typedef struct AllocatorMemoryRequirements {
  ...
  TheForge::ResourceMemoryUsage usage;
  void *pUserData;
...
void AllocatorQuickSort(Iterator beg, Iterator end, Compare cmp) {
  if (beg < end) {
    Iterator
        it = AllocatorQuickSortPartition<Iterator, Compare>(beg, end, cmp);
...
struct AllocatorMutexLock {
 public:
  AllocatorMutexLock(Os::Mutex& mutex, bool
  useMutex) :
      m_pMutex(useMutex
               ? &
                   mutex : RESOURCE_NULL
 ...
 */
template<
    typename IterT, typename
    KeyT,
    typename CmpT
>
static IterT AllocatorBinaryFindFirstNotLess(IterT beg, IterT end, const KeyT
& key,
                                             CmpT cmp
) {
...
static void *AllocatorMalloc(size_t size) { return malloc(size); }

static void AllocatorFree(void *ptr) { free(ptr); }
...
static T *AllocatorAllocateArray(size_t count) {
  return (T *)
      AllocatorMalloc(sizeof(T) * count);
}
...
template<
    typename T
>
static void VectorInsert(AllocatorVector<T>
                         & vec,
                         size_t index,
                         const T& item) {
  vec.
...
template<
    typename T
>
static void VectorRemove(AllocatorVector<T>
                         & vec,
                         size_t index
) {
  vec.
...
template<
    typename T
>
AllocatorPoolAllocator<T>::AllocatorPoolAllocator(size_t
                                                  itemsPerBlock):
...
typename AllocatorPoolAllocator<T>::ItemBlock& AllocatorPoolAllocator<T>::CreateNewBlock() {
  ItemBlock newBlock = {
      new(AllocatorAllocateArray<Item>(m_ItemsPerBlock)) Item(),
      0};
...
AllocatorRawList<T>::AllocatorRawList()
    : m_ItemAllocator(128),
      m_pFront(RESOURCE_NULL), m_pBack(RESOURCE_NULL), m_Count(
        0) {
}
...
template<
    typename T
>
AllocatorListItem<T> *AllocatorRawList<T>::PushBack(const T
                                                    & value) {
...
template<
    typename T
>
AllocatorListItem<T> *AllocatorRawList<T>::PushFront(const T
                                                     & value) {
...
template<
    typename T
>
AllocatorListItem<T> *AllocatorRawList<T>::InsertBefore(ItemType *pItem, const T
& value) {
...
template<
    typename T
>
AllocatorListItem<T> *AllocatorRawList<T>::InsertAfter(ItemType *pItem, const T
& value) {
 ...
 public:
  ...
  class iterator {
   ...
   private:
    ...
    friend
    class AllocatorList<T>;
    friend
    class AllocatorList<T>::const_iterator;
  };
  ...
  class const_iterator {
   public:
    ...
    const_iterator(
        const iterator
        & src) : m_pList(src.m_pList), m_pItem(src.m_pItem) {}
   ...
   private:
    const_iterator(
        const AllocatorRawList<T> *pList,
        const AllocatorListItem<T> *pItem) : m_pList(pList), m_pItem(pItem) {}
...
struct ResourceAllocation {
 public:
  ...
  const { return m_Size; }
  void *GetUserData()
  const { return m_pUserData; }
  void SetUserData(void *pUserData) { m_pUserData = pUserData; }
  AllocatorSuballocationType GetSuballocationType()
  const { return m_SuballocationType; }

  AllocatorBlock *GetBlock()
  const {
    ASSERT(m_Type == ALLOCATION_TYPE_BLOCK);
    return m_BlockAllocation.m_Block;
  }
  uint64_t GetOffset()
  const { return (m_Type == ALLOCATION_TYPE_BLOCK) ? m_BlockAllocation.m_Offset : 0; }
...
class AllocatorBlock {
 public:
  ...
  AllocatorBlock(MetalResourceAllocator *hAllocator);
  ...
  void Destroy(MetalResourceAllocator *allocator);
...
struct AllocatorBlockVector {
  ...
  AllocatorBlockVector(MetalResourceAllocator *hAllocator);
  ~AllocatorBlockVector();

  bool IsEmpty()
  const { return m_Blocks.empty(); }
  ...
  void AddStats(AllocatorStats *pStats, uint32_t memTypeIndex, uint32_t memHeapIndex)
  const;

#if RESOURCE_STATS_STRING_ENABLED
  void PrintDetailedMap(class AllocatorStringBuilder
                        & sb) const;
#endif
 ...
 private:
  MetalResourceAllocator *m_hAllocator;
};
...
struct TheForge_ResourceAllocator {};

// Main allocator object.
struct MetalResourceAllocator : public TheForge_ResourceAllocator {
  bool m_UseMutex;
  ...
  Os::Mutex m_BlocksMutex[RESOURCE_MEMORY_TYPE_NUM_TYPES];

  // Each vector is sorted by memory (handle value).
  typedef AllocatorVector<ResourceAllocation *>
  ...
  Os::Mutex m_OwnAllocationsMutex[RESOURCE_MEMORY_TYPE_NUM_TYPES];

  MetalResourceAllocator(const AllocatorCreateInfo *pCreateInfo);
  ~MetalResourceAllocator();

  uint64_t GetPreferredBlockSize(TheForge::ResourceMemoryUsage memUsage, uint32_t memTypeIndex) const;
  ...
  bool AllocateMemory(
      ...
      & info, const AllocatorMemoryRequirements& resourceAllocMemReq,
 ...
 private:
  ...
  AllocateMemoryOfType(
      const MTLSizeAndAlign
      & mtlMemReq, const AllocatorMemoryRequirements& resourceAllocMemReq,
  ...
  for (size_t i = 0; i < strLen; ++i) {
    ...
    if (ch == '\'') {
      ...
      switch (ch) {
        ...
        case '\t': Add("\\t");
          break;
        default: ASSERT(0 && "Character not currently supported.");
          break;
      }
    }
  }
...
static void AllocatorPrintStatInfo(AllocatorStringBuilder
                                   & sb, const AllocatorStatInfo& stat) {
  sb.Add("{ \"Allocations\": ");
  sb.
      AddNumber(stat
                    .AllocationCount);
...
static void InitStatInfo(AllocatorStatInfo
                         & outInfo) {
  memset(&outInfo, 0, sizeof(outInfo));
  outInfo.
      SuballocationSizeMin = UINT64_MAX;
  outInfo.
      UnusedRangeSizeMin = UINT64_MAX;
}

static void CalcAllocationStatInfo(AllocatorStatInfo
                                   & outInfo, const AllocatorBlock& alloc) {
  outInfo.
      AllocationCount = 1;

  const uint32_t rangeCount = (uint32_t) alloc.m_Suballocations.size();
  outInfo.
      SuballocationCount = rangeCount - alloc.m_FreeCount;
  outInfo.
      UnusedRangeCount = alloc.m_FreeCount;

  outInfo.
      UnusedBytes = alloc.m_SumFreeSize;
  outInfo.
      UsedBytes = alloc.m_Size - outInfo.UnusedBytes;

  outInfo.
      SuballocationSizeMin = UINT64_MAX;
  outInfo.
      SuballocationSizeMax = 0;
  outInfo.
      UnusedRangeSizeMin = UINT64_MAX;
  outInfo.
  ...
  for (
      AllocatorSuballocationList::const_iterator suballocItem = alloc.m_Suballocations.cbegin();
      suballocItem != alloc.m_Suballocations.
          cend();
      ++suballocItem) {
    const AllocatorSuballocation&
        suballoc = *suballocItem;
    if (suballoc.type != RESOURCE_SUBALLOCATION_TYPE_FREE) {
      outInfo.
          SuballocationSizeMin = RESOURCE_MIN(outInfo.SuballocationSizeMin, suballoc.size);
      outInfo.
          SuballocationSizeMax = RESOURCE_MAX(outInfo.SuballocationSizeMax, suballoc.size);
    } else {
      outInfo.
          UnusedRangeSizeMin = RESOURCE_MIN(outInfo.UnusedRangeSizeMin, suballoc.size);
      outInfo.
          UnusedRangeSizeMax = RESOURCE_MAX(outInfo.UnusedRangeSizeMax, suballoc.size);
    }
  }
}

// Adds statistics srcInfo into inoutInfo, like: inoutInfo += srcInfo.
static void AllocatorAddStatInfo(AllocatorStatInfo
                                 & inoutInfo, const AllocatorStatInfo& srcInfo) {
  inoutInfo.AllocationCount += srcInfo.
      AllocationCount;
  inoutInfo.SuballocationCount += srcInfo.
      SuballocationCount;
  inoutInfo.UnusedRangeCount += srcInfo.
      UnusedRangeCount;
  inoutInfo.UsedBytes += srcInfo.
      UsedBytes;
  inoutInfo.UnusedBytes += srcInfo.
      UnusedBytes;
  inoutInfo.
      SuballocationSizeMin = RESOURCE_MIN(inoutInfo.SuballocationSizeMin, srcInfo.SuballocationSizeMin);
  inoutInfo.
      SuballocationSizeMax = RESOURCE_MAX(inoutInfo.SuballocationSizeMax, srcInfo.SuballocationSizeMax);
  inoutInfo.
      UnusedRangeSizeMin = RESOURCE_MIN(inoutInfo.UnusedRangeSizeMin, srcInfo.UnusedRangeSizeMin);
  inoutInfo.
      UnusedRangeSizeMax = RESOURCE_MAX(inoutInfo.UnusedRangeSizeMax, srcInfo.UnusedRangeSizeMax);
}

static void AllocatorPostprocessCalcStatInfo(AllocatorStatInfo
                                             & inoutInfo) {
  inoutInfo.
      SuballocationSizeAvg =
      (inoutInfo.SuballocationCount > 0) ? AllocatorRoundDiv<uint64_t>(inoutInfo.UsedBytes,
                                                                       inoutInfo.SuballocationCount)
                                         : 0;
  inoutInfo.
      UnusedRangeSizeAvg =
...
id <MTLHeap> ResourceAllocation::GetMemory() const {
  return (m_Type == ALLOCATION_TYPE_BLOCK) ? m_BlockAllocation.m_Block->m_hMemory :
         nil;
}

id <MTLBuffer> ResourceAllocation::GetResource() const {
  if (m_Type == ALLOCATION_TYPE_OWN) { return NULL; }
  return (m_SuballocationType == RESOURCE_SUBALLOCATION_TYPE_BUFFER) ? m_BlockAllocation.m_Block->m_Buffer :
         nil;
}

uint32_t ResourceAllocation::GetMemoryTypeIndex()
const {
  return (m_Type == ALLOCATION_TYPE_BLOCK) ? m_BlockAllocation.m_Block->m_MemoryTypeIndex : m_OwnAllocation.
      m_MemoryTypeIndex;
}
  ...
  switch (m_Type) {
    case ALLOCATION_TYPE_BLOCK:
      if (m_BlockAllocation.m_Block->m_pMappedData != RESOURCE_NULL) {
        return (char *) m_BlockAllocation.m_Block->m_pMappedData + m_BlockAllocation.
            m_Offset;
      } else {
        return
            RESOURCE_NULL;
...
struct AllocatorSuballocationItemSizeLess {
  bool operator()(const AllocatorSuballocationList::iterator lhs, const AllocatorSuballocationList::iterator rhs)
  const {
    return lhs->size < rhs->size;
  }
  bool operator()(const AllocatorSuballocationList::iterator lhs, uint64_t rhsSize)
  const { return lhs->size < rhsSize; }
};

AllocatorBlock::AllocatorBlock(MetalResourceAllocator *hAllocator) :
...
void AllocatorBlock::Destroy(MetalResourceAllocator *allocator) {
...
bool AllocatorBlock::Validate()
const {
  if ((m_hMemory == nil) || (m_Size == 0) || m_Suballocations.
      empty()
      ) {
    return
        false;
...
// True if previous visisted suballocation was free.
  ...
  for (
      AllocatorSuballocationList::const_iterator suballocItem = m_Suballocations.cbegin();
...
// Two adjacent free suballocations are invalid. They should be merged.
    ...
    if (currFree) {
      calculatedSumFreeSize += subAlloc.
          size;
      ++
          calculatedFreeCount;
...
bool AllocatorBlock::CheckAllocation(
    uint64_t bufferImageGranularity, uint64_t allocSize, uint64_t allocAlignment, AllocatorSuballocationType allocType,
    AllocatorSuballocationList::const_iterator freeSuballocItem, uint64_t *pOffset)
const {
  ASSERT(allocSize > 0);
  ASSERT(allocType != RESOURCE_SUBALLOCATION_TYPE_FREE);
...
// Apply RESOURCE_DEBUG_MARGIN at the beginning.
  if ((RESOURCE_DEBUG_MARGIN > 0) && freeSuballocItem != m_Suballocations.
      cbegin()
      ) {
...
// Make bigger alignment if necessary.
  if (bufferImageGranularity > 1) {
    ...
    while (prevSuballocItem != m_Suballocations.
        cbegin()
        ) {
      --
          prevSuballocItem;
      const AllocatorSuballocation&
          prevSuballoc = *prevSuballocItem;
      if (
          AllocatorBlocksOnSamePage(prevSuballoc
                                        .offset, prevSuballoc.size, *pOffset, bufferImageGranularity)) {
        if (
            AllocatorIsBufferImageGranularityConflict(prevSuballoc
                                                          .type, allocType)) {
...
// If conflict exists, allocation cannot be made here.
  if (bufferImageGranularity > 1) {
    ...
    ++
        nextSuballocItem;
    while (nextSuballocItem != m_Suballocations.
        cend()
        ) {
      const AllocatorSuballocation&
          nextSuballoc = *nextSuballocItem;
      if (
          AllocatorBlocksOnSamePage(*pOffset, allocSize, nextSuballoc
              .offset, bufferImageGranularity)) {
        if (
            AllocatorIsBufferImageGranularityConflict(allocType, nextSuballoc
                .type)) {
          return
              false;
        }
...
bool AllocatorBlock::IsEmpty()
const {
  return (m_Suballocations.
      size()
      == 1) && (m_FreeCount == 1);
}

void AllocatorBlock::Alloc(const AllocatorAllocationRequest
                           & request,
                           AllocatorSuballocationType type, uint64_t
                           allocSize) {
  ASSERT(request.freeSuballocationItem != m_Suballocations.end());
  AllocatorSuballocation&
      suballoc = *request.freeSuballocationItem;
...
// it to become used.
  UnregisterFreeSuballocation(request
...
// If there are any free bytes remaining at the end, insert new free suballocation after current one.
  if (paddingEnd) {
    ...
    paddingSuballoc.
        size = paddingEnd;
    paddingSuballoc.
        type = RESOURCE_SUBALLOCATION_TYPE_FREE;
...
void AllocatorBlock::PrintDetailedMap(class AllocatorStringBuilder
                                      & sb) const {
  sb.Add("{\n\t\t\t\"Bytes\": ");
  sb.
      AddNumber(m_Size);
  ...
  for (
    ...
    sb.
        AddString(RESOURCE_SUBALLOCATION_TYPE_NAMES[suballocItem->type]);
    sb.Add(", \"Size\": ");
    sb.
        AddNumber(suballocItem
                      ->size);
    sb.Add(", \"Offset\": ");
    sb.
        AddNumber(suballocItem
...
AllocatorBlockVector::AllocatorBlockVector(MetalResourceAllocator *hAllocator) :
...
void AllocatorBlockVector::PrintDetailedMap(class AllocatorStringBuilder
                                            & sb) const {
  for (
      size_t i = 0;
      i < m_Blocks.size();
...
void AllocatorBlockVector::AddStats(AllocatorStats *pStats, uint32_t memTypeIndex, uint32_t memHeapIndex)
const {
  for (
...
bool resourceAllocFindMemoryTypeIndex(
    MetalResourceAllocator *allocator,
  ...
  *pMemoryTypeIndex = UINT32_MAX;
  ...
  switch (pSuballocType) {
    case RESOURCE_SUBALLOCATION_TYPE_BUFFER:
      if (pMemoryRequirements->usage == TheForge::RESOURCE_MEMORY_USAGE_GPU_ONLY) {
        *pMemoryTypeIndex = RESOURCE_MEMORY_TYPE_DEFAULT_BUFFER;
      } else if (
          pMemoryRequirements->usage == TheForge::RESOURCE_MEMORY_USAGE_CPU_ONLY ||
              pMemoryRequirements->usage == TheForge::RESOURCE_MEMORY_USAGE_CPU_TO_GPU) {
        *pMemoryTypeIndex = RESOURCE_MEMORY_TYPE_UPLOAD_BUFFER;
      } else if (pMemoryRequirements->usage == TheForge::RESOURCE_MEMORY_USAGE_GPU_TO_CPU) {
...
MetalResourceAllocator::MetalResourceAllocator(const AllocatorCreateInfo *pCreateInfo)
    :
    m_UseMutex((pCreateInfo
        ->flags & RESOURCE_ALLOCATOR_EXTERNALLY_SYNCHRONIZED_BIT) == 0),
    m_Device(pCreateInfo
                 ->device),
  ...
  static const uint64_t DefaultLargeHeapBlockSize = (64 * 1024 * 1024); // 64 MB
  static const uint64_t DefaultSmallHeapBlockSize = (16 * 1024 * 1024); // 16 MB

  m_PreferredLargeHeapBlockSize = (pCreateInfo->preferredLargeHeapBlockSize != 0)
                                  ? pCreateInfo->preferredLargeHeapBlockSize
                                  : DefaultLargeHeapBlockSize;
  m_PreferredSmallHeapBlockSize = (pCreateInfo->preferredSmallHeapBlockSize != 0)
                                  ? pCreateInfo->preferredSmallHeapBlockSize
                                  : DefaultSmallHeapBlockSize;
  for (
      size_t i = 0;
    ...
    for (
        size_t j = 0;
        j < RESOURCE_BLOCK_VECTOR_TYPE_COUNT;
        ++j) {
      m_pBlockVectors[i][j] = new(AllocatorAllocate<AllocatorBlockVector>()) AllocatorBlockVector(this);
      m_pOwnAllocations[i][j] =
          new(AllocatorAllocate<AllocationVectorType>()) AllocationVectorType();
    }
  }
}

MetalResourceAllocator::~MetalResourceAllocator() {
...
extern AllocatorHeapProperties const gHeapProperties[RESOURCE_MEMORY_TYPE_NUM_TYPES];

bool MetalResourceAllocator::AllocateMemoryOfType(
    const MTLSizeAndAlign
    & mtlMemReq, const AllocatorMemoryRequirements& resourceAllocMemReq,
    uint32_t memTypeIndex,
    AllocatorSuballocationType
  ...
  const bool hostVisible = resourceAllocMemReq.usage != TheForge::RESOURCE_MEMORY_USAGE_GPU_ONLY;
  ...
  if (ownMemory) {
    ...
    for (size_t allocIndex = 0;
         allocIndex < blockVector->m_Blocks.size();
         ++allocIndex) {
      ...
      if (pBlock->CreateAllocationRequest(
          ...
          mtlMemReq.size, mtlMemReq.align, suballocType, &allocRequest)) {
        ...
        *pAllocation = new(AllocatorAllocate<ResourceAllocation>()) ResourceAllocation;
        (*pAllocation)->InitBlockAllocation(pBlock,
                                            allocRequest.offset,
                                            mtlMemReq.align,
                                            mtlMemReq.size,
                                            suballocType,
    ...
    if ((resourceAllocMemReq.flags & RESOURCE_MEMORY_REQUIREMENT_NEVER_ALLOCATE_BIT) != 0) {
      ...
      AllocatorBlock *const pBlock = new(AllocatorAllocate<AllocatorBlock>()) AllocatorBlock(this);
      pBlock->Init(memTypeIndex,
                   (RESOURCE_BLOCK_VECTOR_TYPE) blockVectorType,
                   mem, allocInfo.size, false, NULL);
...
bool MetalResourceAllocator::AllocateOwnMemory(
    uint64_t size, AllocatorSuballocationType suballocType, uint32_t memTypeIndex, bool map, void *pUserData,
    ResourceAllocation **pAllocation) {
  ASSERT(pAllocation);
...
uint64_t MetalResourceAllocator::GetPreferredBlockSize(TheForge::ResourceMemoryUsage memUsage, uint32_t memTypeIndex) const {
  return gHeapProperties[memTypeIndex].mBlockSize;
}

bool MetalResourceAllocator::AllocateMemory(
...
void MetalResourceAllocator::FreeMemory(ResourceAllocation *allocation) {
  ASSERT(allocation);
...
void MetalResourceAllocator::CalculateStats(AllocatorStats *pStats) {
...
void MetalResourceAllocator::UnmapPersistentlyMappedMemory() {
...
bool MetalResourceAllocator::MapPersistentlyMappedMemory() {
...
void MetalResourceAllocator::GetAllocationInfo(ResourceAllocation *hAllocation, ResourceAllocationInfo *pAllocationInfo) {
...
void MetalResourceAllocator::FreeOwnMemory(ResourceAllocation *allocation) {
...
void MetalResourceAllocator::PrintDetailedMap(AllocatorStringBuilder
                                         & sb) {
  bool ownAllocationsStarted = false;
  for (
      size_t memTypeIndex = 0;
      memTypeIndex < GetMemoryTypeCount();
      ++memTypeIndex) {
    AllocatorMutexLock ownAllocationsLock(m_OwnAllocationsMutex[memTypeIndex], m_UseMutex);
    for (
        uint32_t blockVectorType = 0;
        blockVectorType < RESOURCE_BLOCK_VECTOR_TYPE_COUNT;
        ++blockVectorType) {
      ...
      if (pOwnAllocVector->
        ...
        for (
          ...
          sb.Add(", \"Type\": ");
          sb.
              AddString(RESOURCE_SUBALLOCATION_TYPE_NAMES[hAlloc->GetSuballocationType()]);
          sb.Add(" }");
        }
...

  {
    ...
    for (
      ...
      for (
          ...
          blockVectorType < RESOURCE_BLOCK_VECTOR_TYPE_COUNT;
          ++blockVectorType) {
        if (m_pBlockVectors[memTypeIndex][blockVectorType]->
            IsEmpty()
            == false) {
          ...
          m_pBlockVectors[memTypeIndex][blockVectorType]->
              PrintDetailedMap(sb);
...
bool CreateAllocator(const AllocatorCreateInfo *pCreateInfo, MetalResourceAllocator **pAllocator) {
  ASSERT(pCreateInfo && pAllocator);
  RESOURCE_DEBUG_LOG("resourceAllocCreateAllocator");
  *pAllocator = new(AllocatorAllocate<MetalResourceAllocator>()) MetalResourceAllocator(pCreateInfo);
  return true;
}

void DestroyAllocator(MetalResourceAllocator *allocator) {
...
void resourceAllocBuildStatsString(MetalResourceAllocator *allocator, char **ppStatsString, uint32_t detailedMap) {
  ...
  char *const pChars = new(AllocatorAllocateArray<char>((len + 1))) char;
  if (len > 0) {
...
void ResourceAllocUnmapPersistentlyMappedMemory(MetalResourceAllocator *allocator) {
  ASSERT(allocator);
...
bool ResourceAllocMapPersistentlyMappedMemory(MetalResourceAllocator *allocator) {
  ASSERT(allocator);
...
void ResourceAllocFreeMemory(MetalResourceAllocator *allocator, ResourceAllocation *allocation) {
  ASSERT(allocator && allocation);
...
void ResourceAllocGetAllocationInfo(MetalResourceAllocator *allocator,
...
void ResourceAllocSetAllocationUserData(MetalResourceAllocator *allocator, ResourceAllocation *allocation, void *pUserData) {
  ASSERT(allocator && allocation);
